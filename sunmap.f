...
C     Reads output, map_data.prn, from the VB Imove program which estimates the 
C     sunspot counts and motions on the solar disk using Lat, Long
C     (or X, Y cordinates.) Data from Makemap (map_data.sql)  
C********************************************************************************
      ...
      DOUBLE PRECISION INF(8)
C COMMON /CS2TIM/ is needed for Microsoft FORTRAN compiler
      COMMON Y,T,ETA,ETADEL,CONETA,ATA1,ATA1AT,SCRAT,A
C      COMMON /CS2TIM/ Y,T,ETA,ETADEL,CONETA,ATA1,ATA1AT,SCRAT,A
      CHARACTER*5 STRING
      CHARACTER*1 ADAT,COM
      CHARACTER*25 CAPT,LABEL
C identify program to user
      WRITE(*,*)' Program simulates sunspot pore evolution   '
      WRITE(*,*)' from Umbra, Penumbra structure using  '
      WRITE(*,*)' State transition NLDE  '
      WRITE(*,*)' '
C     use either random values for sunspot Pores
C     or data from makemap output of sunspot evolution, map_data.prn
C     or data from MySQL (map_data.prn) 
      WRITE(*,*)' Choose'
      WRITE(*,*)' map_data Y,  Random = R'
      WRITE(*,*)' >'
      READ(*,*)ADAT
      IF (ADAT.EQ.'Y') THEN
         OPEN (25,FILE='map_data.PRN',ERR=400)
C         READ(25,80,ERR=400) NREC,N,LABEL
         WRITE(*,*) ' >'
         WRITE(*,*) NREC,N,LABEL
      ENDIF
C		open output to a csv file 
        OPEN (3,FILE='SUN.CSV',ERR=400)
80    FORMAT(I8,I8,A75)
...
C    set N to Carrington rotations 3*27 days
      ...
      WRITE(*,*)' >'
      READ(*,*)beta
      WRITE(3,*)' Number of Umbra seen' ,beta
      WRITE(*,*)' Percentage of Umbra with Penumbra - delta '
      WRITE(*,*)' >'
      READ(*,*)delta
      WRITE(3,*)' Percentage of Umbra per group',delta
      WRITE(*,*)' Enter number of total sunspots seen'
      WRITE(*,*)' >'
      READ(*,*)rr
      WRITE(3,*)' Number of total sunspots' ,rr
      rr = ((rr + 1) / 100.)
      WRITE(*,*)' Percentage of sunspots in Umbra - gamma '
      WRITE(*,*)' >'
      READ(*,*)g
      WRITE(3,*)' Percentage of of sunspots in Umbra' ,g
      WRITE(*,*)' Estimate percent of Umbra in groups P(0<P<1.0)'
      WRITE(*,*)' >'
      READ(*,*)P
      Pr=(1.-P)/10
      WRITE(3,*)' Estimate percent Umbra in groups',Pr
C       Pr=P
      WRITE(*,*)' Estimate duration of Umbra in days - K(1<K<85)'
      WRITE(*,*)' >'
      READ(*,*)K
C      IF(K.GT.27)K=27
      WRITE(3,*)' Duration of Umbra in days ' ,K
...
3     format(1X,A4,A1,6X,A5,A1,7X,A5,A1,2X,A4,A1,4X,A4)

C main loop to collect data and compute group evolution

      DO WHILE(NEXT.GE.1)
C                       get next batch of data
200     READ(25,80,ERR=400) NREC,N,LABEL
        NBATCH = N
        IF(NBATCH < 100) NBATCH = 3000
        NREC = 1
        WRITE(*,*) ' Continue (1), Stop (0)'
        WRITE(*,*)' >'
        READ(*,*) NEXT
        IF(NEXT.NE.1) GOTO 50
C       use random values for sunspot evolution.
        IF(ADAT .NE. 'Y') CALL RNSTNR(Y,N)
C       loop through the Carrington Rotations NBATCH times
        LOC=10
        NEWY = 1
          DO 100 IREC = NREC, NBATCH
             IF(ADAT.EQ.'Y') THEN
               READ(25,90,ERR=200) ICSG,iday,ICWSA,ILNS,ICLD
90             FORMAT(5(I8))
               CUA = SQRT(FLOAT(ILNS)*FLOAT(ICLD))/ICWSA
               IF(iday.GT.last)last=iday
               SUSQ = CUA
C  set exposure model parameters
               RINF=rr*CUA
               freq=g*CUA
               C=beta*CUA
               PG=P*CUA
...
C            WRITE(*,90) ICSG,last,ICWSA,ILNS,ICLD,ICUA
        write(3,13) iday,COM,SUSQI,COM,POSQ*SUSQS,COM,CUA,COM,ICWSA
13           format(1X,I4,2(A1,F12.4),A1,F8.4,A1,I4)
           NEWY = NEWY + 1
100       CONTINUE
C next batch in the data file
C display number of Carrington Rotatins 27 days
        NREC = NREC+N
        IF(ADAT.EQ.'Y')THEN
          IF(last.GT.85)last=85
          N=(last*3)-3
        ENDIF
        NBATCH = NBATCH+N
        GOTO 500
...
400     CONTINUE
        WRITE(*,410) LOC,IREC
410     FORMAT(3(/),'  Error on reading map_data.prn : ',
...
c if timestep = 1 then SPUR subroutine will read the POLYPARA.RDP file.
c if timestep > 1 then SPUR subroutine will read the TIMESTEP.DAT file
c written as a result of the SPUR equations from timestep 0.
c
c Beta values come from MOVES subroutine (input for polygon
...
c model.
c
c The input file POLYPARA.RDP is start condition at t = 0 for each polygon.
c The POLYPARA.RDP file is created using SPURRDP.F
...
C
      ...
      IF (timestep.EQ.0) THEN
        OPEN (9,FILE='TIMENEW.DAT',STATUS='OLD')
        OPEN (7,FILE='POLYPARA.DAT',STATUS='OLD')
        LOC = 1
...
22      continue
        ...
        close (9)
      ELSE
        open (8,file='timeold.dat',status='OLD')
        open (9,file='TIMENEW.DAT',status='OLD')

         do 24 k = 1, icell
         LOC = 3
         READ(8,99,ERR=300)POLYID,S,E,I,R,A,G,M,B
...
C
        ...
        do 124 k = 1, icell
         LOC = 4
         READ(9,99,ERR=300)POLYID,S,E,I,R,A,G,M,B
         WRITE(8,99,ERR=400)POLYID,S,E,I,R,A,G,M,B
124     continue
...
c
        do 200 k = 1, icell
200        write(*,205) inf(k)
205     format(2X,8(F3.2))
        write(*,206) B
206     format(2x,F6.4)
c  comment or delete this goto 10 when this becomes a subroutine
c
c        goto 10
        goto 999
C
C      Error messages
C
C270    write(*,*) '  Error opening POLYPARA.RPD'
C       goto 999
C280    write(*,*) '  Error opening TIMEOLD.DAT'
C       goto 999
C290    write(*,*) '  Error opening TIMENEW.DAT'
C       goto 999
300    write(*,305) LOC
305    format('  Error reading file at LOC = ',I2)
       goto 999
400    write(*,305) LOC
405    format('  Error writing file at LOC = ',I2)
